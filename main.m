...
% Requires the mapping and image processing tool boxes.
...
% The constants have been set for the "North Dakota to Central Quebec*"
% data set available at: http://eol.jsc.nasa.gov/Videos/CrewEarthObservationsVideos/Videos_Aurora.htm
% To run this with code with that data set, simply download the
% high res image files, and place them in a folder called HighRes in the
% parent folder of the code files. 
...
% Only look for stars in the first N rows starting from the top
% Change this value if too much of the surface or not enough of the
% starts are getting included. 
% TODO: Ideally this should use the segmentation explicitly.
row_limit = 1200;
...
[ horizon_image cut_rgb_image aurora ] = kmeans_segment(first_file, FULL);
...
% speed a lot.
trails = startrails(path, mission, start_image_num, num_images, img_size, resize_factor*row_limit, resize_factor);
...
% This uses a basic flood fill algorithm that has been modified to extact
% all star trails and their start and end points. 
...
clear lsq;
lsq = leastsq_global(img_size, ISS_period, focal_length);

[ minimal_residual y x z ] = lsq.calc_residuals(data);

% The angle off the ISS orbit is given by minimal_residual(3), and the
% camera rotation is given by minimal_residual(2).
...
display(['Angle off ISS orbit: ' num2str(minimal_residual(3))]);
display(['Camera rotation: ' num2str(minimal_residual(2))]);

%% Project onto map.

display('Step 5 of 5: Showing the results on a map...');

% Note this code is very approximate and does not account for things like
% the curvature of the earth and perspective transformations.
% This is given mainly as a proof of concept, and needs more work.
